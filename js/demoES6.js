class drawDemo {
	...
	addEvent() {
		...
		_this.demoCanvas.addEventListener("mousedown", e => {
		...
		document.addEventListener('mouseup', e => {
			...
			//线条粗细
		$(".wide-thin").on("click", "i[_type]", (e) => {
			let _type = $(e.target).attr("_type");
			let numval = $(".num-val").val();
		...
		//橡皮擦与形状
		$(".tools").on("click", "[tool]", (e) => {
				let _tool = $(e.target).attr("tool");
				$(".tools [tool]").removeClass("clickT");
				$(e.target).addClass("clickT");
				_this.flag = _tool;
			})
			//颜色
